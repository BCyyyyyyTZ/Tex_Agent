...
class LatexError:
    ...
    message: 人类可读的错误摘要
    file: 报错关联文件（尽量从日志解析得到，可能为空）
    line: 报错行号（1-based；可能为 0 表示未知）
    context: 报错行附近的上下文片段（用于规则与 LLM 提取命令名）
    raw: 原始错误片段（用于 history 与 LLM 提示）
    """
    message: str
    file: str = ""
    line: int = 0
...
class LatexAutoFixTool(BaseTool):
    ...
    关键保证：
    - 永远不直接修改用户的原始项目目录
    - 每轮修复都会记录到 history，便于审计与回滚
    - 当错误定位到被 \\input/\\include 的子文件时，会优先修改对应子文件
      （但“缺宏包”类修复会写入入口文件导言区）
    """
    def __init__(
        self,
        *,
        ...
        """初始化 LaTeX 自动修复工具，并配置 LLM 参数与是否启用 LLM 修复。"""
        super().__init__(
            name="latex_autofix",
    ...
    def _load_gemini_client_class(self):
        """从项目源码动态加载 GeminiClient 类，避免导入阶段对可选依赖产生硬耦合。"""
    ...
    def _decode_bytes(self, b: bytes) -> str:
        """将 subprocess 输出的 bytes 按常见编码解码为 str（失败则 replace）。"""
        if not b:
            return ""
    ...
    def _which(self, exe: str) -> Optional[str]:
        """查找可执行文件路径（优先 PATH，Windows 下额外探测常见 MiKTeX 安装目录）。"""
        p = shutil.which(exe)
        if p:
            return p
    ...
    def _project_copy(self, src_dir: Path, dst_dir: Path, extra_ignore_top: Optional[set[str]] = None) -> None:
        """复制项目到工作目录，并忽略常见无关目录（如 .git/venv/outputs）。"""
        extra = set(extra_ignore_top or set())

        def _ignore(_: str, names: list[str]) -> set[str]:
            """返回 copytree 需要忽略的顶层目录集合（用于加速复制并避免污染工作区）。"""
            deny = {
                ".git",
                ".hg",
                ".svn",
    ...
    def _compile(
        ...
        *,
        ...
        """在 work_dir 中编译入口 tex，并返回一次编译的结构化结果与关键路径。"""
    ...
    def _extract_first_error(self, text: str) -> LatexError:
        """从编译输出中提取首个关键错误（message/file/line/context/raw）。"""
        s = (text or "").strip("\ufeff")
        if not s:
    ...
    def _find_target_file(self, work_dir: Path, err_file: str, fallback: Path) -> Path:
        """根据日志给出的 err_file 在副本项目内定位需要修改的目标文件。"""
        if err_file:
    ...
    def _read_lines(self, p: Path) -> list[str]:
        """按 UTF-8 读取文本为“保留换行符”的行列表（解码失败则 replace）。"""
        try:
    ...
    def _insert_package(self, lines: list[str], pkg: str) -> tuple[bool, list[str]]:
        """在导言区 \\begin{document} 前插入 \\usepackage{pkg}（若已存在则不改）。"""
        joined = "".join(lines)
    ...
    def _escape_underscores_on_line(self, line: str) -> tuple[bool, str]:
        """在疑似文本模式下将未转义的下划线替换为 \\_（尽量避免破坏数学与链接命令）。"""
        if "\\_" in line:
            return False, line
        if "$" in line:
            return False, line
    ...
    def _deterministic_fix(
        ...
        *,
        ...
        """对常见报错执行确定性修复（加宏包、转义下划线、切换引擎、开启 shell-escape）。"""
    ...
    def _apply_llm_edits_to_file(self, target: Path, edits: list[dict[str, Any]], raw: str) -> Optional[dict[str, Any]]:
        """校验并应用 LLM 返回的多处行区间替换（从后往前应用以避免行号漂移）。"""
    ...
    def _llm_fix(
        ...
        *,
        ...
        """调用 LLM 生成多处修复建议，并将建议应用到目标 .tex 文件。"""
    ...
    def run(
        ...
        """复制工程副本并迭代编译/修复，直到编译通过或达到 max_iters 上限。"""
        try:
            src_project = None
            src_main = None
